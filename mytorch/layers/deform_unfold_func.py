...
class DeformUnfoldFunction(Function):
    @staticmethod
    def forward(ctx, input, offset, kernel_size, stride=1, padding=0, dilation=1, deformable_groups=1, im2col_step=64):
        ...
        output = input.new_empty(
            DeformUnfoldFunction._output_size(input, ctx.kernel_size, ctx.padding, ctx.dilation, ctx.stride)
        )
        ...
        if not input.is_cuda:
            ...
            deform_modules_cuda.deform_unfold_forward(
                input,
                offset,
                output,
                ctx.bufs_[0],
    ...
    def backward(ctx, grad_output):
        ...
        if not grad_output.is_cuda:
            ...
            if ctx.needs_input_grad[0] or ctx.needs_input_grad[1]:
                ...
                deform_modules_cuda.deform_unfold_backward_input(
                    input,
                    offset,
                    grad_output,
                    grad_input,
        ...
        return grad_input, grad_offset, None, None, None, None, None, None, None
    ...
    def _output_size(input, kernel_size, padding, dilation, stride):
        ...
        if not all(map(lambda s: s > 0, img_output_size)):
            raise ValueError(
                "convolution input is too small (output would be {})".format("x".join(map(str, img_output_size)))
            )
        output_size = (
            input.size(0),
            channels * kernel_size[0] * kernel_size[1],
            img_output_size[2] * img_output_size[3],
        )
        return output_size
