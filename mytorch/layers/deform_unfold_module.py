...
class DeformUnfold(nn.Module):
    def __init__(self, kernel_size, stride=1, padding=0, dilation=1, deformable_groups=1, bias=False):
        assert not bias
    ...
    def forward(self, input, offset):
        return deform_unfold(
            input, offset, self.kernel_size, self.stride, self.padding, self.dilation, self.deformable_groups
        )
